...
type Baris = { key: number; mode: "existing" | "baru" };
...
export function MitraPicker({
  ...
  const [baris, setBaris] = useState<Baris[]>(
    awal.length ? awal.map((_, i) => ({ key: i, mode: "existing" as const })) : [{ key: 0, mode: "existing" }],
  );
  ...
  return (
    <div>
      ...
      <button
        ...
        style={inputGaya}
        onClick={() => setBaris((s) => [...s, { key: kunciBerikutnya++, mode: "existing" }])}
      >
        + Tambah Mitra Lain
      </button>
