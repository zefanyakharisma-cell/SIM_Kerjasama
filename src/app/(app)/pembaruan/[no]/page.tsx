...
/**
 * One renewal, end to end (PRD §9.3–§9.6, Design §5.3, §5.9, §5.11).
 *
 * Everything the renewal needs is on one screen because it is one task with two
 ...
 * permits.
 *
 * The gate banner states its own reasoning. An upload control that is simply
 * absent, with no explanation, is the version of this screen that generates
 * support tickets.
 */
...
const GERBANG: Record<string, { label: string; warna: string; jelas: string }> = {
  menunggu: {
...
export default async function DetailPembaruan({
  params,
}: {
  ...
  async function isiEvaluasiFakultas(formData: FormData) {
    "use server";
    await kirimEvaluasiFakultas(
      Number(formData.get("no_evaluasi")),
      bacaJawaban(formData),
      noDokumen,
    );
  }
  ...
  async function bukaUlang(formData: FormData) {
    "use server";
    await bukaUlangEvaluasi(Number(formData.get("no_evaluasi")), noDokumen);
  }

  async function unggahDraf(formData: FormData) {
    "use server";
    await buatProposalPerpanjangan(
      noDokumen,
      String(formData.get("file") ?? "") || null,
    );
  }
  ...
  return (
    <div className="max-w-3xl">
      <header className="mb-5">
        <Link href="/pembaruan" className="text-xs underline">
          ← Kembali ke daftar pembaruan
        </Link>
        <h1 className="mt-2 text-lg font-semibold" style={{ color: "var(--midnight)" }}>
          Pembaruan · <span className="no-dokumen">{r.no_dokumen ?? "—"}</span>
        </h1>
        <p className="text-sm" style={{ color: "var(--text-secondary)" }}>
          {r.nama_mitra} · {r.unit_pengusul ?? "unit tidak tercatat"} · berakhir{" "}
          {r.tanggal_berakhir} ({r.sisa_hari} hari lagi)
        </p>
      </header>
      ...
      {r.gerbang === "split" && io ? (
        <section
          ...
          style={{ borderColor: "var(--action-danger)" }}
        >
      ...
      {r.id_proposal_penerus ? (
        <p className="mb-6 text-sm">
          ...
          <Link href={`/kerja-sama/${r.id_proposal_penerus}`} className="underline">
            buka proposal
          </Link>
        </p>
      ) : null}
